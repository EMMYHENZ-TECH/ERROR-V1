...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./ERRORMedia/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./ERRORMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./ERRORMedia/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./ERRORMedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./ERRORMedia/trash/${filename}.mp4`), unlink(`./ERRORMedia/trash/${filename}.gif`)])
    return buffer5
}
